    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                imgs = self.denorm(x_concat.data.cpu())

                for person in imgs:
                  uid = uuid.uuid1()
                  for i in range(0,9):
                    img = person[:,:,196*i:(196*i)+196]
                    result_path=(os.path.join(self.result_dir,str(i),str(uid)+".jpg"))
                    try:
                      os.mkdir(os.path.join(self.result_dir,str(i)))
                    except:
                      print()
                    save_image(img, result_path)
